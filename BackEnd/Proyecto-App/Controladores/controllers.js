const updateAvailableMoney = require('../services/updateAvailableMoney');
...
const deductInvestment = async (req, res) => {
    try {
        const investment = req.body;
        const result = await db.collection('investments').insertOne(investment);
        // Update available money
        await updateAvailableMoney(-result.ops[0].amount);
        res.status(201).json({ message: 'Investment deducted successfully', data: result.ops[0] });
    } catch (error) {
        res.status(500).json({ message: 'Server error deducting investment', error });
    }
};

const deductIncome = async (req, res) => {
    try {
      const income = req.body;
      const result = await db.collection('incomes').insertOne(income);
      await updateAvailableMoney(-result.ops[0].amount);
      res.status(201).json({ message: 'Income deducted successfully', data: result.ops[0] });
    } catch (error) {
      res.status(500).json({ message: 'Server error deducting income', error });
    }
  };
